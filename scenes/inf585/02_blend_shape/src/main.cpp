...
buffer<buffer<vec3>> d_storage; //Store all the differences between one key-frame face and the neutral one
...
void initialize_data()
{
	...
	//    faces_storage[k_face].position[k] - refers to the k-th vertex of the pose k_face (for k_face>0)
        ...
        buffer<vec3> d1 = faces_storage[1].position - faces_storage[0].position;
        buffer<vec3> d2 = faces_storage[2].position - faces_storage[0].position;
        buffer<vec3> d3 = faces_storage[3].position - faces_storage[0].position;
        buffer<vec3> d4 = faces_storage[4].position - faces_storage[0].position;
        buffer<vec3> d5 = faces_storage[5].position - faces_storage[0].position;

        d_storage.push_back(d1);
        d_storage.push_back(d2);
        d_storage.push_back(d3);
        d_storage.push_back(d4);
        d_storage.push_back(d5);
...
void update_blend_shape()
{
    ...
    for (size_t i=0 ; i<face_mesh.position.size() ; i++){
        bi0 = faces_storage[0].position[i];
        new_p = bi0;
        for (size_t k=0 ; k<N_face-1 ; k++){
            wk = weights[k];
            dik = d_storage[k][i];
            new_p += wk*dik;
        }
        face_mesh.position[i] = new_p;
    }

    buffer<vec3> new_face_positions = face_mesh.position;
    face.update_position(new_face_positions);
