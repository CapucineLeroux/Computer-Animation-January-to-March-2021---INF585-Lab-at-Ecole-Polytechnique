...
int main(int, char* argv[])
{
	...
	std::cout<<"Start animation loop ..."<<std::endl;
        ...
        // Modify the frequency of the event
        timer.event_period = 0.25;
...
void create_new_particle(float current_time)
{
	...
	new_particle.p0 = {0,0,0};
        new_particle.v0 = {0.8f*std::sin(theta),0.8f*std::cos(theta),rand_interval(2,5)};
...
void display_scene(float current_time)
{
	vec3 const g = {0,0,-9.81f}; // gravity constant
        float const alpha = 0.7; // energy loss after collision
	...
	for (size_t k = 0; k < N; ++k)
	{
		...
		// **************************************************************** //
                float t_collision = -2.*v0[2]/g[2];
                vec3 p_collision = 0.5f*g*t_collision*t_collision + v0*t_collision + p0;
                vec3 p;
                ...
                if (t<=t_collision){
                    ...
                    p = 0.5f*g*(t-t_collision)*(t-t_collision) + alpha*v0*(t-t_collision) + p_collision; // models the second parabola
                }
