...
void compute_deformation()
{
	...
	// Compute skinning deformation
        bool const rectangle_bend_z = ImGui::Button("Bend z###RectangleBendZ");
        ...
        if (rectangle_bend_z || rectangle_bend_zx || rectangle_twist_x){
...
void display_interface()
{
	...
	bool const marine_walk = ImGui::Button("Walk"); ImGui::SameLine();
        bool const marine_dance = ImGui::Button("Dance"); ImGui::SameLine();
	bool const marine_idle = ImGui::Button("Idle");

	GLuint texture_id = mesh_drawable::default_texture;
        if (marine_run || marine_walk || marine_dance || marine_idle) load_skinning_data("../assets/marine/", skeleton_data, rig, new_shape, texture_id);
        ...
        if(marine_dance) load_skinning_anim("../assets/marine/anim_dance/", skeleton_data);
        if(marine_idle) load_skinning_anim("../assets/marine/anim_idle/", skeleton_data);
        if (marine_run || marine_walk || marine_dance || marine_idle) {
		update=true;
