...
void display_grid();
buffer<buffer<vec3>> fill_weights(buffer<vec3> position);
...
buffer<grid_3D<float>> w_mijk; // Contains the pre-computed weights for the ffd
...
void create_new_surface() // call this function every time we change of surface
{
	...
	require_shape_update = true;
        w_mijk = fill_weights(shape.position,grid);
}
