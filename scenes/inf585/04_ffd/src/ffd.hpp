...
vcl::buffer<vcl::grid_3D<float>> fill_weights(vcl::buffer<vcl::vec3> const position, vcl::grid_3D<vcl::vec3> const grid);

// Compute the FFD deformation
void ffd_deform(vcl::buffer<vcl::vec3>& position, vcl::grid_3D<vcl::vec3> const& grid, vcl::buffer<vcl::grid_3D<float>> w_mijk);
