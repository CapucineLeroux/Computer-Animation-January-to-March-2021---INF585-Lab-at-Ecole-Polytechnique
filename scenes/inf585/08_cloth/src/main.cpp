		...
		if(user.gui.run){
			...
			size_t const N_substeps = 5;
                        for(size_t k_substep=0; k_substep<N_substeps; ++k_substep){
                                ...
                                compute_forces(cloth.forces, cloth.position, cloth.velocity, cloth.normal, cloth.side_length, cloth.parameters, user.gui.wind_magnitude);
				numerical_integration(cloth.position, cloth.velocity, cloth.forces, m, dt);
                                apply_constraints(cloth.position, cloth.normal, cloth.velocity, cloth.positional_constraints, obstacles);
...
void initialize_data()
{
	...
	sphere = mesh_drawable(mesh_primitive_sphere());
        cylinder = mesh_drawable(mesh_primitive_cylinder(obstacles.cylinder_radius,obstacles.p1,obstacles.p2));
        sphere1 = mesh_drawable(mesh_primitive_sphere(obstacles.cylinder_radius,obstacles.p1));
        sphere2 = mesh_drawable(mesh_primitive_sphere(obstacles.cylinder_radius,obstacles.p2));
...
void initialize_cloth()
{
	...
	cloth.positional_constraints.clear();
        /**for (size_t i=0 ; i<N_cloth ; i++){
            cloth.positional_constraints[cloth.position.index_to_offset(i,0)] = cloth.position(int(i),0);
        }*/
